...
(function () {
  ...
  // WhatsApp fallback numbers for upload-failure error banner. Different
  // line per language so the right Laura answers.
  var WHATSAPP_FALLBACK = {
    en: { num: '4915510894317', text: 'Hey Laura, I got a problem applying on the website. That\'s the reason I\'m reaching out directly.' },
    es: { num: '4915511502140', text: 'Hola Laura, tuve un problema al aplicar en la web. Por eso te escribo directamente.' }
  };
  ...
  function onSubmit(e) {
    ...
    uploadAllPhotos(uploadedImages).then(function (urls) {
      ...
      return fetch(JP_WEBHOOK_URL, {
        ...
        mode: 'cors'
      }).then(function () { return { payload: payload, allFailed: allFailed }; }, function () {
        // n8n often returns CORS-opaque on success - treat as ok.
        return { payload: payload, allFailed: allFailed };
      });
    }).then(function (result) {
      // If all photos failed: show the WhatsApp fallback banner so the
  ...
  // ============================================================
  // WhatsApp fallback banner (replaces plain text error on photo /
  // network failure). Renders an inline button with pre-filled message.
  // ============================================================
